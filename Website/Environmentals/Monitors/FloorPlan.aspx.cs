...
{
    public partial class FloorPlan : EnvironmentalMonitor.Website.Abstracts.AbstractSecurityPage
    {
        ...
        public string SaveJsonDataUrl
        {
            get
            {
                ...
                if ((!string.IsNullOrEmpty(this.RoomId)) && (!string.IsNullOrEmpty(this.MachineId)))
                {
                    result = string.Format("{0}Environmentals/Monitors/FloorPlanSaveJson.aspx?roomId={1}&machineId={2}", Variable.VirtualRootPath, this.RoomId, this.MachineId);
                }
        ...
        {
            this.RoomId = this.ParameterString(FloorPlan.ROOM_ID);
            this.MachineId = this.ParameterString(FloorPlan.MACHINE_ID);
