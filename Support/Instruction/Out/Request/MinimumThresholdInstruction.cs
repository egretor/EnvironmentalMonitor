...
namespace EnvironmentalMonitor.Support.Instruction.Out.Request
{
    ...
    /// 设置探测器阀值指令
    /// </summary>
    public class MinimumThresholdInstruction : EnvironmentalMonitor.Support.Instruction.AbstractInstruction
    {
        ...
        /// 构造函数
        /// </summary>
        public MinimumThresholdInstruction()
        {
        }
        ...
        public MinimumThresholdInstruction(List<Detector> detectors)
        {
            if (detectors != null)
            {
                int address = 0;
                ...
                for (int i = 0; i < detectors.Count; i++)
                {
                    if (detectors[i] != null)
                    {
                        byte[] values = detectors[i].EncodeMinimum();
        ...
        public override List<AbstractInstruction> ErrorAnswer()
        {
            ...
            if (Variable.Debug)
            {
                AbstractInstruction instruction = new Out.Response.MinimumThresholdErrorInstruction();
        ...
        public override List<AbstractInstruction> RightAnswer()
        {
            ...
            if (Variable.Debug)
            {
                AbstractInstruction instruction = new Out.Response.MinimumThresholdSuccessInstruction();
